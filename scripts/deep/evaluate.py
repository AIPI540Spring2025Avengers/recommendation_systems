...
def evaluate_recommendation_system():
    ...
    _, test_User_data, _, test_Hotel_data = data_preprocess(pd.read_csv(os.path.join(data_dir, "train.csv")))
    
    # Load user model from different possible paths
    model_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "models")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    user_tower = torch.load(os.path.join(model_dir, "user_tower.pth"), weights_only=False)
    hotel_tower = torch.load(os.path.join(model_dir, "hotel_tower.pth"), weights_only=False)
